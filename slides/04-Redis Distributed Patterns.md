...
| ----------------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| **TTL too short** | Work still running; lock expires; another client enters → **double execution**.          | TTL ≥ p99 job time + margin; **extend lease** in-process if safe; or **short lock + idempotent** downstream. |
